...
window.apt = (function() {
  ...
  // ─── STEP FLOW: transient connector cue between two CENTER-STAGE
  // solution cards (Given→Step1→Step2...), distinct from stackLineGrow
  // (which grows a PERMANENT line in the left stack column). This one
  // grows in and then fades back out — a passing "the reasoning continues
  // here" cue in the middle of the screen, not a structural element.
  // Requires a pre-positioned `.step-flow-line` element as a sibling of
  // the solution cards (see design-system.css) — position/size it by hand
  // per question, same as a .stack-line, centered on the cards' shared
  // horizontal axis. Fire at the SAME time as the morphToStack call that
  // starts the outgoing card's exit, so the line bridges outgoing → the
  // incoming card's cardEnter.
  // Usage: apt.stepFlow(tl, "#flow-given-1", 29.5);
  function stepFlow(tl, selector, time) {
    const GROW = 0.5;
    const HOLD = 0.6;
    const FADE = 0.35;
    tl.fromTo(selector,
      { scaleY: 0, opacity: 0 },
      { scaleY: 1, opacity: 0.6, duration: GROW, ease: "power2.out" },
      time);
    tl.to(selector, { opacity: 0, duration: FADE, ease: "power2.in" }, time + GROW + HOLD);
  }
