...
window.apt = (function() {
  ...
  function morphToStack(tl, fromSel, toSel, time) {
    ...
    if (from && to && root) {
      ...
      // A prior version here used scaleX/scaleY separately to match the
      // stack card's true width AND height - that's what q10 (the proven
      // reference build) does NOT do, and independent-axis scaling is
      // exactly the difference that made this composition's morph land
      // visibly off from the real stack card in the studio's render path,
      // even though the underlying x/y math checks out in a plain browser.
      // Match q10's known-good uniform scale instead.
